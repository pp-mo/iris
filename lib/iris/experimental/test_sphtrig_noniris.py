...
DERIVED FROM : test_spherical_geometry

Attempt to make a version that does not depend on Iris
...
def monotonic(array):
    """
    Substitute for iris.util.monotonic.

    'strict' is assumed : i.e. no zero-steps
    """
    diffs = np.diff(array)
    return all(np.sign(diffs) == np.sign(diffs[0]))


class FakeIrisTest(unittest.TestCase):
    """A 'fake' IrisTest, to provide extra array assert methods."""
...
class TestConverts(FakeIrisTest):
...
class TestSphPoint(FakeIrisTest):
...
class TestSphGcSeg(FakeIrisTest):
...
class TestSphPolygon(FakeIrisTest):
