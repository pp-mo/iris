...
#    dims = [len(coord.points) for coord in (x_coord, y_coord)]
    assert y_coord.shape == x_coord.shape
    dims = x_coord.shape
    ...
    lon_bounds, lat_bounds = x_coord.bounds, y_coord.bounds
...
def regrid_2d_conservative_via_esmpy(source_cube, grid_cube):
    """
...
#    dst_coords = i_regrid._get_xy_dim_coords(grid_cube)
    src_coords = _hack_xy_dim_coords(source_cube)
    dst_coords = i_regrid._get_xy_dim_coords(grid_cube)
