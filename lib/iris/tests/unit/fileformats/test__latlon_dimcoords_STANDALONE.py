...
from iris.tests.unit.fileformats._nc_loadtests_support import (
...
"""
    ...
    def test_unrotated_units_wrongname_gridmapping(self):
        # Mismatched "degrees" unit + latlon mapping, treated as 'misc'
        result = self.run_testcase(units="degrees", grid_mapping="latlon")
        self.check_result(result, None, None, "degrees", None)

    def test_unrotated_units_wrongname_axis_gridmapping(self):
        # Mismatched "degrees" unit + latlon mapping, with axis=X
        # No coord at all ???  An odd result, but that is it.
        result = self.run_testcase(
            units="degrees", grid_mapping="latlon", axis="x"
        )
        self.assertEqual(0, len(result.coords()))
    ...
    def test_multifactor(self):
        ...
        # -- that is : standard_name; units; axis; grid_mapping
        ...
        def expected_results(standard_name, units, axis, grid_mapping=None):
            """
            ...
            Returns:
                (
                    expected_coord_standard_name,
                    expected_coord_units,
                    expected_coord_coordsystemtype
                )

            """
            expected_stdname = standard_name
            expected_units = units
            expected_coordsystemtype = grid_mapping
            valid = False
            rotated = False
            ...
            if not valid:
                expected_coordsystemtype = None

            return expected_stdname, expected_units, expected_coordsystemtype
        ...
        axis_opts = [None, "x", "z"]
        gridmap_opts = [None, "latlon", "rotated"]
        gridmap_opts = [None, "latlon"]
        known_errors = {
            (None, None, None, None): None,
            (None, "degrees", "x", None): None,
            (
                None,
                "degrees",
                "x",
                "latlon",
            ): None,  # Truly weird : get NO COORD
            ("longitude", None, None, "latlon"): ("longitude", None, "latlon"),
            ("longitude", None, "x", "latlon"): ("longitude", None, "latlon"),
            ("longitude", None, "z", "latlon"): ("longitude", None, "latlon"),
            (
                "grid_longitude",
                None,
                None,
                ...
                for axis in axis_opts:
                    for gridmap in gridmap_opts:
                        result = self.run_testcase(
                            ...
                            units=units,
                            axis=axis,
                            grid_mapping=gridmap,
                        )

                        case = (std, units, axis, gridmap)
                        if case in known_errors:
                            expects = known_errors[case]
                            ...
                            (
                                ...
                                expect_units,
                                expect_cstype,
                            ) = expected_results(std, units, axis, gridmap)
                        ...
                        context_message = (
                            ...
                            f"cstype={expect_cstype!r}"
                            "\n  got : "
                            f"\nstdname={coord.standard_name!r}, longname={coord.long_name!r}, "
                            f"varname={coord.var_name!r}, units={coord.units!r}, coord_system={coord.coord_system!r}"
                        )
                        self.check_result(
                            result,
                            expect_stdname,
